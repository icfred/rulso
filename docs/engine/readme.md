_Last edited: 2026-05-11 by RUL-23 (post-RUL-68: real `DiscardRedraw` substrate live — `rules.discard_redraw` retires the M2-era `discard_redraw_unimplemented` placeholder across CLI + server)_
...
| `engine/src/rulso/rules.py` | live | round flow phase machine + SHOP phase wiring (RUL-51): `enter_round_start` step-5 SHOP check, `complete_shop`, `apply_shop_purchase`, `shop_purchase_order`. RUL-68 added `discard_redraw(state, player_id, card_ids, *, refill_rng)` + internal `_draw_n` helper (recycle-from-discard pattern mirrors `_refill_hands`); validates phase=BUILD, active-seat, hand-membership, chip-affordability, non-empty `card_ids` — see [round-flow.md](round-flow.md) |
...
| `engine/src/rulso/cli.py` | live | round-by-round CLI runner; RUL-68 retired the `discard_redraw_unimplemented` placeholder — `_drive_build_turn`'s `DiscardRedraw` branch now dispatches through `rules.discard_redraw` and emits `event=turn action=discard_redraw cost=…` — see [cli.md](cli.md) |
