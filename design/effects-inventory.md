...
| `eff.noop` | `NOOP` | `noop` | 0 | `all_matched` | nothing | the deliberate "nothing happens" card; proves dispatcher returns state unchanged without crashing on unknown / null effect path |

**12 cards because**: 5 effect_kind families (chips, vp, status_apply, status_clear, hand_size_modify) × 1-3 cards each + the `noop` sentinel + one `everyone_except_matched` example. Smaller than 12 leaves a kind family untested; larger duplicates the same dispatch path with different magnitudes (a parametrisation concern, not a vocabulary one). MARKED status is intentionally excluded from the starter — it interacts with `EACH PLAYER` SUBJECT scope, which is itself M2-late (per `cards-inventory.md`); coupling them is needless risk for the head fan.
