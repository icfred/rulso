...
| Ticket | PR | Notes |
|---|---|---|
| RUL-68 | #72 | `rules.discard_redraw(state, player_id, card_ids, *, refill_rng) -> GameState` + internal `_draw_n` helper (mirrors `_refill_hands` recycle pattern). 5 ValueError validations (non-BUILD, out-of-turn, unknown card, insufficient chips, empty `card_ids`). `cli._drive_build_turn` retires the `discard_redraw_unimplemented` placeholder; emits `event=turn action=discard_redraw cost=…`. `server._apply_action` routes through the same handler with `refill_rng` threaded via `_take_human_turn` / `_take_bot_turn`. 13 new tests in `test_discard.py` + 1 server round-trip. M2 watchable smoke 6/10 floor preserved (intermediate state perturbed but aggregate floor stable). `test_determinism.py` byte-identical-stdout invariant holds (no baseline shift required — disjoint-rng pattern preserved). |
| RUL-69 | #73 | `client/src/render/{cards,rule,goals,opponents}.ts` — pure renderers. `main.ts` rebuilds `#rule-preview` / `#goals` / `#opponents` / `#actions` on every `StateBroadcast`; action button labels flow through `renderCard` (text, not JSON). Raw envelope log moved behind `<details>`. SUBJECT player ids render absolutely ("Player 0" for "p0"); not seat-relative. Goal `claim_condition → prose` is hand-rolled per hand-over allowance. Floating labels (LEADER/WOUNDED/GENEROUS/CURSED) recomputed client-side per ADR-0001 — **flagged as drift risk; substrate follow-up filed**. NO engine changes; `generated.ts` untouched (`envelopes.ts` re-exports a few extra generated types). 513 engine tests pass; client typecheck/lint/build clean. Worker had no browser in env so the screenshot DoD was deferred to merge reviewer; functional Node ws-client smoke output in PR body confirms all four panels render correctly against `--seed 0 --human-seat 0` round-1 state. |

**Worker hand-back flags addressed**:

- RUL-68: server round-trip test catches the FIRST chip-mutated broadcast after submission; that broadcast may carry `phase=ROUND_START` if the human's discard happened on the last build turn (rule failed/resolved). Test asserts chip drop + hand-size preservation rather than card conservation, since post-discard the engine may add played-fragment cards to `state.discard` in the same broadcast. **Decision**: accepted; the assertion shape is correct given the engine's resolve-after-build sequencing.
- RUL-68: `card_ids` removal walks the hand by id (first match per id); duplicate-id hands behave identically to `_remove_first`. **Decision**: edge case not exercised in current cards.yaml (no duplicate-id cards), but the documented behaviour is correct. No follow-up.
- RUL-69: floating labels recomputed client-side per ADR-0001 — engine doesn't serialise them. Worker correctly flagged as drift risk. **Filing as labels-on-wire substrate ticket** (next dispatchable).
- RUL-69: SUBJECT player ids render absolutely (`p0` → "Player 0") not seat-relative. **Decision**: accepted; seat-relative ("You"/"Opponent N") is a UX polish task for later. Documented inline.
- RUL-69: no browser in worker env; screenshot DoD deferred. **Decision**: accepted; functional Node ws-client smoke is equivalent verification (same envelope shapes; renderer functions are pure). Real-browser smoke deferred to next user playtest.

**Cross-cutting fixes landed via this RUL-23 sweep**:

- `docs/engine/readme.md`: `_Last edited:` bumped; `rules.py` row notes `discard_redraw` + `_draw_n`; `cli.py` row notes the placeholder retirement; `server.py` row notes the `DiscardRedraw` routing; `test_server.py` row notes the new round-trip test; new `test_discard.py` row added
- `docs/client/readme.md`: `_Last edited:` bumped; `main.ts` row notes the panel-rebuild + `renderCard` text labels + `<details>` log; new rows for `render/{cards,rule,goals,opponents}.ts`; smoke-section caveats updated (DiscardRedraw placeholder caveat removed — fixed; floating-labels drift caveat added)
- STATUS.md re-anchored to post-RUL-68/69 (this entry)

### Open follow-ups post-RUL-68/69

- **Labels-on-wire substrate** (NEW, surfaced by RUL-69): engine should serialise the floating-label registry on the wire (additive field on `GameState` or `StateBroadcast`) so clients consume the canonical computation rather than re-implementing ADR-0001's tie-break rules. Drift risk: any future ADR-0001 amendment needs both engine + client updates. **Filing as next dispatchable.**
- **`bots/human` TTY rewire through WS** (final M3 functional, per RUL-64 "Out of scope"): make the engine WS server the canonical "engine" — the CLI gets a `--ws` flag that connects to `rulso-server` and drives a human seat via TTY (same legal-action surface as the browser). **Filing as next dispatchable; parallel-safe with labels-on-wire (cli/server vs engine state).**
- **`_OP_ONLY_COMPARATOR_NAMES` duplication** between `cli.py` and `server.py` — promote to `legality.py` or `effects.py`. Low priority parallel-safe one-PR refactor; not blocking
- **TS type-gen `state.py` coverage** — extend `scripts/regenerate-types.sh` to introspect `rulso.state` directly if rendering needs richer state types beyond what `StateBroadcast.state` already pulls via the closure. Not a blocker
- **`npm run lint` cwd quirk** (RUL-67 worker note) — investigate separately if another contributor bumps it
- **Seat-relative SUBJECT rendering** ("You" / "Opponent N" instead of "Player 0..3") — UX polish; defer to next playtest signal
